...
struct OptionallyStaticStepRange{
    ...
    global function _OptionallyStaticStepRange(
            @nospecialize(start::IntType),
            @nospecialize(step::IntType),
            @nospecialize(stop::IntType)
        )
        return new{typeof(start), typeof(step), typeof(stop)}(start, step, stop)
    end
end
@noinline function OptionallyStaticStepRange(
        @nospecialize(start::IntType),
        ::StaticInt{0},
        @nospecialize(stop::IntType)
    )
...
function OptionallyStaticStepRange(
        @nospecialize(start::IntType),
        step::StaticInt{1},
        @nospecialize(stop::IntType)
    )
    return _OptionallyStaticStepRange(start, step, stop)
end
function OptionallyStaticStepRange(
        @nospecialize(start::IntType),
    ...
    return _OptionallyStaticStepRange(start, step, _steprange_last(start, step, stop))
end
function OptionallyStaticStepRange(start, step, stop)
    return OptionallyStaticStepRange(IntType(start), IntType(step), IntType(stop))
end
function OptionallyStaticStepRange(
        @nospecialize(start::IntType),
        step::Int,
...
function OptionallyStaticStepRange(x::Union{Base.Slice, Base.IdentityUnitRange})
    return OptionallyStaticStepRange(x.indices)
end
function OptionallyStaticStepRange(x::Base.OneTo)
    return _OptionallyStaticStepRange(static(1), static(1), Int(last(x)))
end
function OptionallyStaticStepRange(x::OptionallyStaticUnitRange)
    return _OptionallyStaticStepRange(getfield(x, :start), static(1), getfield(x, :stop))
end
function OptionallyStaticStepRange(x::AbstractUnitRange)
    return _OptionallyStaticStepRange(Int(first(x)), static(1), Int(last(x)))
end
function OptionallyStaticStepRange(x::AbstractRange)
    return _OptionallyStaticStepRange(Int(first(x)), Int(step(x)), Int(last(x)))
end
...
@inline function _steprange_last(
        start::Union{StaticInt, Int},
        step::Union{StaticInt, Int},
        stop::Union{StaticInt, Int}
    )
    return _steprange_last(Int(start), Int(step), Int(stop))
end
...
function Base.checkindex(
    ...
    return (F1::Int <= F2::Int) && (L1::Int >= L2::Int)
end
...
@inline function Base.iterate(x::OptionallyStaticRange, s::Int)
    return Base.isdone(x, s) ? nothing : _next(x, s)
end
...
function Base.axes1(x::OptionallyStaticUnitRange)
    return OptionallyStaticUnitRange(StaticInt(1), length(x))
end
...
function Base.axes1(x::OptionallyStaticUnitRange{StaticInt{F}, StaticInt{L}}) where {F, L}
    return OptionallyStaticUnitRange(StaticInt(1), StaticInt(L - F + 1))
end
...
function Base.axes1(
        x::OptionallyStaticStepRange{
            StaticInt{F}, StaticInt{S}, StaticInt{L},
        }
    ) where {
        F,
        S,
    ...
    return OptionallyStaticUnitRange(StaticInt(1), StaticInt(_range_length(F, S, L)))
end
...
function Base.:(-)(r::OptionallyStaticRange)
    ...
    return (-getfield(r, :start)):s:(-getfield(r, :stop))
end

function Base.reverse(x::OptionallyStaticUnitRange)
    return _OptionallyStaticStepRange(getfield(x, :stop), StaticInt(-1), getfield(x, :start))
end
function Base.reverse(x::OptionallyStaticStepRange)
    return _OptionallyStaticStepRange(getfield(x, :stop), -getfield(x, :step), getfield(x, :start))
end
...
function Base.show(io::IO, ::MIME"text/plain", @nospecialize(r::OptionallyStaticUnitRange))
    return print(io, "$(getfield(r, :start)):$(getfield(r, :stop))")
end
function Base.show(io::IO, ::MIME"text/plain", @nospecialize(r::OptionallyStaticStepRange))
    return print(io, "$(getfield(r, :start)):$(getfield(r, :step)):$(getfield(r, :stop))")
end
...
function Base.Broadcast.axistype(
        r::OptionallyStaticUnitRange{StaticInt{1}},
...
function Base.similar(
    ...
    return Array{T}(undef, map(last, axes))
end
function Base.similar(
        ::Type{<:Array{T}},
        axes::Tuple{
            Base.OneTo, OptionallyStaticUnitRange{StaticInt{1}},
            Vararg{
                Union{
                    Base.OneTo,
                    OptionallyStaticUnitRange{StaticInt{1}},
...
function Base.first(x::OptionallyStaticUnitRange, n::IntType)
    ...
    return OptionallyStaticUnitRange(start, min(start - one(start) + n, getfield(x, :stop)))
end
function Base.first(x::OptionallyStaticStepRange, n::IntType)
    ...
    return OptionallyStaticStepRange(start, s, stop)
end
function Base.last(x::OptionallyStaticUnitRange, n::IntType)
    n < 0 && throw(ArgumentError("Number of elements must be nonnegative"))
    stop = getfield(x, :stop)
    return OptionallyStaticUnitRange(max(stop + one(stop) - n, getfield(x, :start)), stop)
end
function Base.last(x::OptionallyStaticStepRange, n::IntType)
    ...
    s = getfield(x, :step)
    stop = getfield(x, :stop)
    return OptionallyStaticStepRange(max(stop + one(stop) - (n * s), start), s, stop)
end
